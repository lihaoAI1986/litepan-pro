...
class AsyncDatabase:
    ...
    async def initialize(self):
        try:
            ...
            await self._conn.executescript("""
                ...
                -- v3 path-play: STRM 同步期间把 (account_id, remote_path) -> file_id / pick_code
                -- 持久化下来；/d/{account_id}/{path} 入口先查这张表，查不到再走驱动搜索。
                CREATE TABLE IF NOT EXISTS path_file_cache (
                    id              INTEGER PRIMARY KEY AUTOINCREMENT,
                    account_id      INTEGER NOT NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_path_file_cache_account
                    ON path_file_cache(account_id);
    ...
    # ---------- v3 path_file_cache 增删改查 ----------

    async def upsert_path_file_cache(
        self,
        account_id: int,
        remote_path: str,
        file_id: str,
        ...
        """插入或更新一条 (remote_path -> file_id) 记录。"""
        try:
            await self._conn.execute(
                """
                INSERT INTO path_file_cache (
                    account_id, remote_path, file_id, pick_code,
                    file_name, file_size, parent_id, last_seen_at
                ...
                (
                    ...
                    str(remote_path or ""),
                    str(file_id or ""),
                    str(pick_code or "") or None,
                    str(file_name or "") or None,
                    int(file_size or 0) if file_size is not None else None,
                    str(parent_id or "") or None,
                ),
            )
            await self._conn.commit()
            return True
            ...
            try:
                from core.log_manager import get_writer, LogModule
                get_writer(LogModule.DATABASE).warning(f"path_file_cache upsert 失败: {e}")
            except Exception:
                pass
            return False

    async def upsert_path_file_cache_batch(self, rows) -> int:
        """批量 upsert，提升 STRM 同步时的写入效率。"""
        if not rows:
            return 0
        ok = 0
        try:
                ...
                try:
                    await self._conn.execute(
                        ...
                        (
                            ...
                            str(row.get("pick_code") or "") or None,
                            str(row.get("file_name") or "") or None,
                            int(row.get("file_size") or 0) if row.get("file_size") is not None else None,
    ...
    async def get_path_file_cache(self, account_id, remote_path):
        try:
            async with self._conn.execute(
                "SELECT * FROM path_file_cache WHERE account_id = ? AND remote_path = ?",
                (int(account_id), str(remote_path or "")),
            ) as cursor:
    ...
    async def delete_path_file_cache(self, account_id, remote_path) -> bool:
        try:
            await self._conn.execute(
                "DELETE FROM path_file_cache WHERE account_id = ? AND remote_path = ?",
                (int(account_id), str(remote_path or "")),
            )
            await self._conn.commit()
            return True
    ...
    async def delete_path_file_cache_by_account(self, account_id) -> int:
        try:
            cursor = await self._conn.execute(
                "DELETE FROM path_file_cache WHERE account_id = ?",
                (int(account_id),),
            )
            await self._conn.commit()
    ...
    async def delete_path_file_cache_by_prefix(self, account_id, prefix) -> int:
        """删除某前缀下所有缓存：STRM 同步做清理时使用。"""
        if not prefix:
            return 0
        try:
            ...
            cursor = await self._conn.execute(
                "DELETE FROM path_file_cache WHERE account_id = ? AND (remote_path = ? OR remote_path LIKE ?)",
                (int(account_id), normalized, like_pattern),
            )
            await self._conn.commit()
            return cursor.rowcount or 0
        except Exception:
    ...
    async def count_path_file_cache(self, account_id = None) -> int:
        try:
            if account_id is None:
                cursor = await self._conn.execute("SELECT COUNT(*) AS cnt FROM path_file_cache")
            else:
                cursor = await self._conn.execute(
                    "SELECT COUNT(*) AS cnt FROM path_file_cache WHERE account_id = ?",
                    (int(account_id),),
                )
            row = await cursor.fetchone()
