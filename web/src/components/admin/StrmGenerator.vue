  ...
  <div class="strm-generator">
    ...
    <div v-if="activeTab === 'settings'" class="strm-panel">
      <div class="settings-group">
            ...
            <span class="help-icon" @mouseover="strmLinkFormatTooltipVisible = true" @mouseleave="strmLinkFormatTooltipVisible = false">
              ...
              <div class="tooltip" v-show="strmLinkFormatTooltipVisible">
                <div class="tooltip-content">
                  ...
                  <div class="tooltip-body">
                    <p><b>v3 路径直跳格式 (推荐)</b>：STRM 内容形如 <code>http://host:5211/d/1/电影/动作/阿凡达.mkv</code>，302 跳到网盘 CDN 直链，Emby/Infuse 最友好。</p>
                    <p><b>v1 传统格式</b>：通过 file_id + token + sign 拼接，兼容性一般。</p>
                    <p><b>v2 高兼容格式</b>：base64 编码 file_id，适合外网播放异常时尝试。</p>
                  </div>
                </div>
              </div>
            </span>
        ...
        <div class="group-item" v-if="strmSettings.strm_link_format === 'v3'">
          <div class="group-label with-help">
            v3 路径缓存
            <span class="help-icon" @mouseover="strmPathCacheTooltipVisible = true" @mouseleave="strmPathCacheTooltipVisible = false">
              <i class="fas fa-question-circle"></i>
              <div class="tooltip" v-show="strmPathCacheTooltipVisible">
                <div class="tooltip-content">
                  <div class="tooltip-title">v3 路径缓存说明</div>
                  <div class="tooltip-body">
                    <p>开启后 /d/ 入口会把 (path -> CDN 直链) 缓存 30 分钟（可调），命中后秒级 302；</p>
                    <p>同目录其它文件会异步预缓存，首播几乎零延迟。</p>
                    <p>关闭后每次播放都重新走搜索+取直链。</p>
                  </div>
                </div>
              </div>
            </span>
          ...
          <label class="setting-switch">
            <input v-model="strmSettings.strm_path_cache_enabled" type="checkbox">
            <span></span>
          </label>
        </div>
        <div class="group-item" v-if="strmSettings.strm_link_format === 'v3'">
          <div class="group-label">路径缓存 TTL (秒)</div>
          <input v-model.number="strmSettings.strm_path_cache_ttl" type="number" min="60" max="86400" class="group-input" placeholder="1800">
        </div>
        <div class="group-item" v-if="strmSettings.strm_link_format === 'v3'">
          <div class="group-label">路径缓存容量</div>
          <input v-model.number="strmSettings.strm_path_cache_max_size" type="number" min="100" max="50000" class="group-input" placeholder="5000">
        </div>
        <div class="group-item" v-if="strmSettings.strm_link_format === 'v3'">
          <div class="group-label with-help">
            同目录预缓存
            <span class="help-icon" @mouseover="strmPrecacheTooltipVisible = true" @mouseleave="strmPrecacheTooltipVisible = false">
              <i class="fas fa-question-circle"></i>
              <div class="tooltip" v-show="strmPrecacheTooltipVisible">
                <div class="tooltip-content">
                  <div class="tooltip-title">预缓存说明</div>
                  <div class="tooltip-body">
                    <p>播放某个文件时，异步预取同目录下其它兄弟文件的直链到内存。</p>
                    <p>连续点开一集接一集时几乎零延迟。</p>
                  </div>
                </div>
              </div>
          ...
          <label class="setting-switch">
            <input v-model="strmSettings.strm_path_precache_enabled" type="checkbox">
            <span></span>
          </label>
        </div>
        <div class="group-item" v-if="strmSettings.strm_link_format === 'v3'">
          <div class="group-label">302 缓存时间 (秒)</div>
          <input v-model.number="strmSettings.strm_path_redirect_max_age" type="number" min="0" max="86400" class="group-input" placeholder="300">
        </div>
        <div class="group-item">
          <div class="group-label with-help">
            默认扫描间隔（分钟）
...
const strmPathCacheTooltipVisible = ref(false)
const strmPrecacheTooltipVisible = ref(false)
...
const strmSettings = reactive({
  ...
  strm_link_format: 'v3',
  strm_path_cache_enabled: true,
  strm_path_cache_ttl: 1800,
  strm_path_cache_max_size: 5000,
  strm_path_precache_enabled: true,
  strm_path_redirect_max_age: 300
})
...
const strmLinkFormatOptions = [
  { value: 'v3', label: 'v3 路径直跳格式 (推荐，对 Emby/Infuse 兼容性最佳)' },
...
const normalizeStrmLinkFormat = (value) => {
  ...
  return ['v1', 'v2', 'v3'].includes(normalized) ? normalized : 'v3'
}
...
const loadStrmSettings = async () => {
  ...
  if (response.data.success) {
    ...
    if (response.data.data.strm_path_cache_enabled !== undefined) {
      strmSettings.strm_path_cache_enabled = Boolean(response.data.data.strm_path_cache_enabled)
    }
    if (response.data.data.strm_path_cache_ttl !== undefined) {
      strmSettings.strm_path_cache_ttl = Number(response.data.data.strm_path_cache_ttl) || 1800
    }
    if (response.data.data.strm_path_cache_max_size !== undefined) {
      strmSettings.strm_path_cache_max_size = Number(response.data.data.strm_path_cache_max_size) || 5000
    }
    if (response.data.data.strm_path_precache_enabled !== undefined) {
      strmSettings.strm_path_precache_enabled = Boolean(response.data.data.strm_path_precache_enabled)
    }
    if (response.data.data.strm_path_redirect_max_age !== undefined) {
      strmSettings.strm_path_redirect_max_age = Number(response.data.data.strm_path_redirect_max_age) || 300
    }
...
const saveStrmSettings = async () => {
  ...
  const response = await axios.post('/api/admin/strm/settings', {
    ...
    strm_path_cache_enabled: strmSettings.strm_path_cache_enabled,
    strm_path_cache_ttl: strmSettings.strm_path_cache_ttl,
    strm_path_cache_max_size: strmSettings.strm_path_cache_max_size,
    strm_path_precache_enabled: strmSettings.strm_path_precache_enabled,
    strm_path_redirect_max_age: strmSettings.strm_path_redirect_max_age,
