...
class StrmSyncManager:
    ...
    def _build_play_url(
        ...
        v1/v2 仅靠 file_id；v3 用 remote_path 生成 /d/{account_id}/{remote_path} 形式。
        远程调用方若没传 remote_path，v3 会 fallback 到 v1 行为（避免破坏老任务）。
        """
        link_format = (link_format or "v1").lower()
        if link_format == "v3" and remote_path:
            return build_strm_v3_play_url(base_url, account_id, remote_path)
        if link_format == "v2":
    ...
    async def generate_current_directory_strm(
        ...
        for file_id, file_name, _remote_path, _size, _modified, item_relative_dir in selected:
            ...
            if local_exists:
                ...
                created += 1
            # 同步回填 path_file_cache：让 /d/ 入口首次播放也能命中持久化缓存
            try:
                picked = None
                for src_item in items or []:
                    if str(src_item.get("id") or "") == str(file_id):
                        picked = src_item
                        break
                extra = (picked or {}).get("extra") if isinstance(picked, dict) else None
                pick_code = ""
                parent_id = ""
                ...
                await db.upsert_path_file_cache(
                    ...
                    pick_code=pick_code,
                    file_name=file_name,
                    file_size=int((picked or {}).get("size") or 0) if isinstance(picked, dict) else 0,
                    parent_id=parent_id,
                )
            except Exception:
                pass
    ...
    async def _run_task(self, task_id: int, run_mode: str = "auto"):
        ...
        try:
            ...
            async def handle_one(item: Tuple[str, str, str, int, Optional[datetime], PurePosixPath]):
                ...
                # 同步回填 path_file_cache（仅 v3 必要，但记录了也不亏；老格式也兼容）
                try:
                    extra = None
                    pick_code = ""
                    parent_id = ""
                    file_size_local = int(size or 0)
                    try:
                        extra = getattr(item[0] if isinstance(item, tuple) else None, "extra", None)
                    except Exception:
                        extra = None
                    if isinstance(extra, dict):
                        pick_code = str(extra.get("pick_code") or "")
    ...
    def _parse_strm_play_url(self, value: str) -> Optional[Dict[str, Any]]:
        """识别 STRM 里写下的播放 URL 格式：v1 / v2 / v3。"""
        ...
        # v3 path-based: /d/{account_id}/{remote_path}
        v3_match = re.match(r"^/d/(\d+)(/.*)$", path)
        if v3_match:
            return {
                "format": "v3",
                "parts": parts,
                "account_id": int(v3_match.group(1)),
                "remote_path": normalize_strm_remote_path(urllib.parse.unquote(v3_match.group(2) or "")),
    ...
    async def replace_strm_link_format(self, link_format: str) -> Dict[str, int]:
        ...
        for file_path in self._strm_root.rglob("*.strm"):
            ...
            try:
                ...
                # v3 -> 任意：必须先有 file_id；优先查 path_file_cache
                file_id = str(parsed.get("file_id") or "").strip()
                remote_path = str(parsed.get("remote_path") or "").strip()
                if not file_id and remote_path:
                    cached = await db.get_path_file_cache(account_id, remote_path)
                    if cached:
                        file_id = str(cached.get("file_id") or "")
                # 任意 -> v3：必须有 remote_path；优先查 path_file_cache
                if target_format == "v3" and not remote_path and file_id:
                    # 反向查找：取这个账号下任意指向同一 file_id 的 remote_path
                    try:
                        async with db._conn.execute(
                            "SELECT remote_path FROM path_file_cache WHERE account_id = ? AND file_id = ? LIMIT 1",
                            (account_id, file_id),
                        ) as cursor:
                            row = await cursor.fetchone()
                        if row:
                            remote_path = str(row[0] or "").strip()
                    except Exception:
                        pass
                if not file_id:
                    # 没有 file_id 时无法做格式转换（v1/v2 必填）
                    if target_format != "v3":
                        continue
                    if not remote_path:
                        continue
