...
async def _get_strm_settings_data(request: Request) -> Dict[str, Any]:
    ...
    return {
        ...
        # ---- v3 path-based 相关 ----
        "strm_path_cache_enabled": normalize_bool(
            await config_manager.get_async("strm_path_cache_enabled"),
            True,
        ),
        "strm_path_cache_ttl": await _get_strm_number_setting("strm_path_cache_ttl", 1800, 60, 86400),
        "strm_path_cache_max_size": await _get_strm_number_setting("strm_path_cache_max_size", 5000, 100, 50000),
        "strm_path_precache_enabled": normalize_bool(
            await config_manager.get_async("strm_path_precache_enabled"),
            True,
        ),
        "strm_path_precache_batch": await _get_strm_number_setting("strm_path_precache_batch", 100, 0, 1000),
        "strm_path_redirect_max_age": await _get_strm_number_setting("strm_path_redirect_max_age", 300, 0, 86400),
        "strm_path_cache_count": 0,  # 由后续调用覆盖为实际条数
    }
...
class StrmSettingsUpdate(BaseModel):
    ...
    strm_path_cache_enabled: Optional[bool] = None
    strm_path_cache_ttl: Optional[int] = None
    strm_path_cache_max_size: Optional[int] = None
    strm_path_precache_enabled: Optional[bool] = None
    strm_path_precache_batch: Optional[int] = None
    strm_path_redirect_max_age: Optional[int] = None
...
async def update_strm_settings(payload: StrmSettingsUpdate, request: Request, session_data: dict = Depends(require_admin_auth)):
    try:
        ...
        # ---- v3 path cache ----
        if payload.strm_path_cache_enabled is not None:
            await config_manager.set_async("strm_path_cache_enabled", bool(payload.strm_path_cache_enabled))
        if payload.strm_path_cache_ttl is not None:
            ttl = int(payload.strm_path_cache_ttl)
            if ttl < 60 or ttl > 86400:
                return _error_response(message="路径缓存 TTL 必须是 60-86400 秒")
            await config_manager.set_async("strm_path_cache_ttl", ttl)
            try:
                from core.path_download_cache import get_path_download_cache
                get_path_download_cache().update_ttl(ttl)
            except Exception:
                pass
        if payload.strm_path_cache_max_size is not None:
            ms = int(payload.strm_path_cache_max_size)
            if ms < 100 or ms > 50000:
                return _error_response(message="路径缓存容量必须是 100-50000")
            await config_manager.set_async("strm_path_cache_max_size", ms)
            try:
                from core.path_download_cache import get_path_download_cache
                get_path_download_cache().update_max_size(ms)
            except Exception:
                pass
        if payload.strm_path_precache_enabled is not None:
            await config_manager.set_async("strm_path_precache_enabled", bool(payload.strm_path_precache_enabled))
        if payload.strm_path_precache_batch is not None:
            pb = int(payload.strm_path_precache_batch)
            if pb < 0 or pb > 1000:
                return _error_response(message="预缓存批大小必须是 0-1000")
            await config_manager.set_async("strm_path_precache_batch", pb)
        if payload.strm_path_redirect_max_age is not None:
            ma = int(payload.strm_path_redirect_max_age)
            if ma < 0 or ma > 86400:
                return _error_response(message="302 缓存时间必须是 0-86400 秒")
            await config_manager.set_async("strm_path_redirect_max_age", ma)
...
async def path_cache_stats(session_data: dict = Depends(require_admin_auth)):
    try:
        from core.path_download_cache import get_path_download_cache
        cache = get_path_download_cache()
        mem_stats = cache.stats()
        db_count = 0
        try:
            db_count = await db.count_path_file_cache()
        except Exception:
            db_count = 0
        per_account = []
        try:
            ...
            for acct in accounts or []:
                cnt = await db.count_path_file_cache(int(acct.get("id") or 0))
                per_account.append({
        ...
        return _error_response(message=f"获取路径缓存统计失败: {str(e)}")


@router.post("/path-cache/clear")
async def path_cache_clear(payload: dict = None, session_data: dict = Depends(require_admin_auth)):
    try:
        from core.path_download_cache import get_path_download_cache
        cache = get_path_download_cache()
        cache.clear()
        ...
        deleted = 0
        if account_id > 0:
            deleted = await db.delete_path_file_cache_by_account(account_id)
        else:
            # 删全表（用 SQL 直接清空，避免 N 次循环）
            try:
                async with db._conn.execute("DELETE FROM path_file_cache") as cur:
                    deleted = cur.rowcount or 0
                await db._conn.commit()
            except Exception:
                deleted = 0
        return _success_response(
            data={"cleared_memory": True, "deleted_db_rows": deleted, "account_id": account_id},
            message="路径缓存已清空",
        )
    except Exception as e:
        return _error_response(message=f"清空路径缓存失败: {str(e)}")
